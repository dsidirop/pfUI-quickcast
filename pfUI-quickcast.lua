...
pfUI:RegisterModule("QuickCast", "vanilla", function()
    ...
    local namPowerIsSpellInRange_ = _G.IsSpellInRange
    ...
    local function _isSpellTargetInRangeForSpell(spellRawName, spellMaxRange, targetUnit, possiblePrecalculatedDistanceFromTarget)
        ...
        if possiblePrecalculatedDistanceFromTarget == nil then
            possiblePrecalculatedDistanceFromTarget = _getUnitDistanceFromPlayer(targetUnit)
            if possiblePrecalculatedDistanceFromTarget == nil then
                -- if we cant get the distance to the target then we have to assume that its in range and let the spell-cast attempt decide about it
                return true, nil
            end

            possiblePrecalculatedDistanceFromTarget = possiblePrecalculatedDistanceFromTarget - 0.5 -- we optimistically remove half a yard from the distance
        end
    ...
    local function _setTargetIfNeededAndCast(
        ...
        local distanceFromTargetCalcedOnce, spellId, spellBookType, canBeUsed, isSpellCastOnSelfOnly, eventualTarget, spellRawName, isInstantSpell, minRange, maxRange
        ...
        for _, spell in spellsArray do
            ...
            if canBeUsed
                    and not isSpellCastOnSelfOnly
                    and proper_target ~= _player
                    and maxRange < 80
            then
                if distanceFromTargetCalcedOnce ~= nil then -- first time
                    canBeUsed = distanceFromTargetCalcedOnce <= maxRange -- this is faster because we avoid an entire function call from the 2nd spell onwards
                else
